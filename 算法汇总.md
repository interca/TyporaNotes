...
void solve() {
  ...
  while(q.size()){
    ...
    while(size --){
       ...
       if(w == 1 &&(node -> left != NULL || node ->right != NULL))f = 1;
       if(node -> left != NULL)q.push(node -> left);
       if(node ->right != NULL)q.push(node -> right);
       if(node ->left == NULL && node ->right != NULL)f = 1;
       if(node ->left == NULL || node ->right == NULL)w = 1;
       if(node -> left == NULL && node -> right == NULL)w = 1;
    }
    v.push_back(v1);
  }
  for(int i = 0 ; i < v.size() ; i ++){
    if(i != v.size() - 1){
      if(pow(2,i) != v[i].size())f = 1;
    }
    for(int j = 0 ; j < v[i].size() ;  j ++){
      cout<<v[i][j];
      if(j != v[i].size() - 1 || i != v.size() - 1)cout<<" ";
    }
  }
  cout<<endl;
  if(f == 0)cout<<"YES";
...
void dijkstra(){
    ...
    while(true){
        ...
        vist[t] = 1;
        for(int i = 1 ; i<= n ; i ++){
            if(vist[i] == 0 && dist[t] + g[t][i] < dist[i]){
                dist[i] = dist[t] + g[t][i];
            }
        }
    }
...
int g[N],e[N],w[N],ne[N],ide,vist[N],dist[N];
int n,m;
int mx = 100000000;
...
void dijkstra(){
    ...
    while(heap.size()){
        ...
        int distant = k.first;
        heap.pop();
        if(vist[t] == 1)continue;
        vist[t] = 1;
        for(int i = g[t] ; i != -1 ; i = ne[i]){
            int j = e[i];
            int value = w[i];
            if(value + dist[t] < dist[j]){
                dist[j] = value + dist[t];
                heap.push({dist[j],j});
            }
        }
...
一个人从一个点出发，经过所有点然后回到原点，只能走一次每个点，求最短路

>这里可以先定义一个数组，dp(i)(j)表示目的地到j  经过点集合i的最短路
>然后可以分解成子问题
>最后加上目的地到原点的距离，然后取一个最小值就行了
>如果是可以重复经过某个点，可以用floyd  来取代graph数组

![微信截图_20230424202321](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230424202321.png)
...
>```
>1: i 代表着目前已经填充了 i - 1 个数字, 目前准备填充第 i 位，mask 则记录了 i 之前已经填充了哪些数字（二进制表示集合）。
>但是需要注意一点的是 dp[i][mask] 缓存的结果，其实是上述代码中的 res, 并不是前 i 个填充的结果，而是在选了前 i 个数字之后，后续递归计算返回的结果，
>缓存的结果是后续还有多少种填充方式是满足题意的（前面的只是限制条件，比如前面已经选了 4，2 那么后续就不能选 4 和 2 了）
>
>2: 通过 1 的分析，我们知道了前 i 个数字只是限制条件，或者说 mask 只是限制条件，dp 缓存的是 mask 限制条件下的结果。但是这个限制条件可能是重复的，
>以 420 为例，前两位可以选 2，4 也可以选 4，2。 它们的 mask 是一样的，那么可以使用同一个缓存的答案吗？答案是否定的，因为如果先选了 2，4 那么第三位可以取 (0,1,3,5,6,7,8,9)。此时 dp[1][20] = 8 但是如果先选了 4，2 的话，就满足了 isLimit 的条件，那么第三位只可以取 0 了，否则就会大于 420 这个数字。所以此时是不可以使用前面 2，4 缓存的数据的 (dp[1][20] = 8)。
>而是要去重新算一下（会得到 1）。所以使用缓存与正确的结果相差 8 - 1 = 7；这样就导致了输出结果 330 比预期结果 323 大 7。
>
>3: 再讨论下如果不满足 isLimit 条件下的，重复 mask 可以共用缓存吗？
>    同理前两位数字可以取 1，2。也可以选择 2，1。此时 mask 也是相同的，但是我们可以发现如果先选择 1，2 的话，第三位可以选择 0-9 中任意跟1，2不重复的数字即可，而 2，1 同理也可以选择 0-9 中任意跟1，2不重复的数字即可，因为这两种情况下第三位无论怎么选都不会大于 420 。所以先算出来的 1，2 的缓存，2，1是可以使用的。
>```
...
int  dfs(int i,int mask,bool islimit,bool isnum){
       ...
       int res = 0;
       //记忆化的过程，减少搜索次数
       if(islimit == false && isnum && dp[i][mask] != -1)return dp[i][mask];
       if(isnum == false){
           //选择跳过
...
![微信截图_20230328132730](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230328132730.png)
...
![微信截图_20230328180802](C:\Users\waili\Desktop\usual\微信截图\ACWING\微信截图_20230328180802.png)
...
![](C:\Users\waili\Desktop\usual\微信截图\ACWING\微信截图_20230328180508.png)
