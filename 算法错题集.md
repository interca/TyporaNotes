...
### 18、[得到整数零需要执行的最少操作数](https://leetcode.cn/problems/minimum-operations-to-make-the-integer-zero/)

>首先执行k此操作， 每一次让a 减去2的i次方加b，最终能不能让b变成0，求出最小值
>枚举操作次数，每一次都要加b，所以我们先把b*i减去得到c，问题变成操作i次能不能让c变成0
>如果c是小于0，不管减去多少都是小于0，直接break
>然后验证i的合法性，i要大于等于c中1的个数并且小于等于c的值
>证明：
>因为每次最少都会减去2的0次方，也就是减去1，所以i最大只能操作c次
>i最小就是每次都把c二进制中的1除去

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230625234528.png)
...
public:
    int makeTheIntegerZero(int num1, int num2) {
         for(long long i = 0 ; i <= 1000 ; i ++){
             long long c = (long long)a - (long long)b*i;
             if(c < 0)break;
             if( __builtin_popcountll(c)<= i && i <= c)return i;
         }
        return -1;
    }
};
...
>我们可以反着求，在一个i到j的子数组中找到小于等于1的个数，然后j - i + c就是新增的
>每次取到一个子数组，可以用一个vist来记录每个数有没有访问过，如果有，那么就插入到
>那个数的后面,c++，然后找出这个数相邻的两个数，决定c的变化

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230702180519.png)
...
>思路
>首先我们可以构造一个三维dp (i) (j)(k)
>表示到了当前位置，构造的字母以i开头，j结尾的长度
>所以转移方程就可以列出来了

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230625234207.png)
...
public:
    int minimizeConcatenatedLength(vector<string>& words) {
        ...
        a[0][words[0][0] - 'a'][words[0][words[0].size() - 1] - 'a'] = words[0].size();
        for(int i = 1 ; i < n ; i ++){
            string s = words[i];
            int c = s.size();
            int d1 = s[0] - 'a';
            ...
            for(int j = 0 ; j <26 ; j ++){
                for(int k = 0 ; k < 26 ; k ++){
                    if(d2 == j){
                        a[i][d1][k] = min(a[i][d1][k],a[i - 1][j][k] + c - 1);
                    }else a[i][d1][k] = min(a[i][d1][k],a[i - 1][j][k] + c);
                    if(d1 == k){
                        a[i][j][d2] = min(a[i][j][d2],a[i - 1][j][k] + c - 1);
                    }else {
                         a[i][j][d2] = min(a[i][j][d2],a[i - 1][j][k] + c);
                    }
                }
            }
        }
...
>思路：
>用递归处理，首先dfs(i,mask,islimit,isnum)表示前i个数包括了mask集合，并且从i开始填数的方案数
>我们用二进制表示一个数是否在集合中，比如0 1 2 在集合中，那么就是1 1 1 ->mask
>islimit是前面的填法有没有限制，比如n = 123  当第一位不填，第二为，第三位没有限制
>isnum表示前面是否构造了数，如果前面全部跳过，就是false
>
>如果前面没有构造数，可以选择跳过
>然后开始遍历，注意起始位置和上下界

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230321164608.png)
...
int  dfs(int i,int mask,bool islimit,bool isnum){
       ...
       //构造上界
       int up  = islimit == true?s[i] - '0':9;
       for(int w = 1 - (int)(isnum) ; w <= up ; w ++){
           //不在集合中
           if((mask >> w & 1) == 0){
             res += dfs(i + 1,mask | (1<<w),w == up && islimit,true);
           }
       }
       return res;
   } 
...
>这里用了贪心思想，先把猫从大到小排序，然后在dfs
>dfs可以分为把这只猫放在已经有的车或者新开一辆车

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230329180949.png)
...
![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230319011645.png)

```cpp
class Solution {
public:
    long long judge(vector<int>&ranks,long long mid){
        int n = ranks.size();
        long long sum = 0;
        for(int i = 0 ; i < n ; i ++){
            sum += (sqrt(1.0*mid / ranks[i]));
        }
        return sum;
}    
...
![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230326133911.png)
...
public:
    ...
    void get_primes(){
      ...
      for(int i=2;i<=n;i++){
        if(st[i] == 0){
            primes.push_back(i);
          for(int j=i;j<=n;j+=i) st[j]=1;//可以用质数就把所有的合数都筛掉；
        }
    }
}
    bool primeSubOperation(vector<int>& nums) {
        ...
        for(int i =  n - 2 ; i >= 0 ; i --){
            ...
            if(l == -1 || primes[l] >= nums[i])return false;
            nums[i] -= primes[l];
        }
        return true;
    }
...
>排序，找到比数小的最右边的下标，然后前缀和分开处理

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230326150057.png)
...
public:
    vector<long long> minOperations(vector<int>& nums, vector<int>& queries) {
        ...
        for(int i = 0 ; i < m ; i++){
            ...
            while(l <= r){
                int mid = l + r >> 1;
                if(nums[mid] <= k)l = mid  + 1,index = mid;
                else r = mid - 1;
            }
                long long s1 = abs((long long)queries[i] * (index + 1) - pre[index + 1]);
                long long s2 = abs((long long)queries[i] * (n - index - 1) - (pre[n] - pre[index  + 1]));
                v.push_back((s1+s2));
        }
        return v;
    }
...
public:
    vector<long long> minOperations(vector<int>& nums, vector<int>& queries) {
        ...
        for(int i = 0 ; i < m ; i++){
            ...
            index = l;
            if(index == -1){
                v.push_back(abs((long long)queries[i] * n - pre[n]));
            }else {
                long long s1 = abs((long long)queries[i] * (index + 1) - pre[index + 1]);
                long long s2 = abs((long long)queries[i] * (n - index - 1) - (pre[n] - pre[index  + 1]));
                v.push_back((s1+s2));
...
>我们可以把区间分成n份，计算末尾是i,长度大于等于f的最大值
>我们可以把这段的值减去avg
>i从f开始取，然后左端点是i - f + 1
>所以此时这段的值是s[i] - s[l - 1]    并且 l >= 1 && l <= i - f + 1 
>l - 1 >= 0 && l <= i - f 
>我们要求s[l - 1]的最小值，所以每次k往后移动的时候要求一次最小值
>最后判断总和是否大于等于0

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230404121002.png)
...
>二分加贪心，枚举mid,然后排序从左往右扫一遍

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230601125506.png)

```cpp
class Solution {
public:
    int m;
    bool check(vector<int>&price,int mid){  
      int c = 1;
      int n = price.size();
      int pre = price[0];
      for(int i = 1  ; i < n ;  i ++){
...
给一个数组，然后给一个m，求连续子数组长度不超过m的异或值
>思路
>我们从前往后变量,右端点为r,所以左端点>=r - m + 1
>我们可以求前缀和，求出s[r],和s[l - 1]这样子就可以得到l 到 r的异或总和
>这样转换为求两个数的异或最大值也就是给出i  然后左端点是 i - m 
>我们可以考虑用树，设当前右端点为x,把x看成31位的二进制，然后树
>的左边树0，右边是1，每次取出最高位，和树的儿子匹配，如果由和
>当前最高位相反的儿子，就往那个儿子走，否则走另一条路，所以可以
>用前缀树实现，由于限制了m，我们必须要能够删除前缀树种的值，所以
>要设置一个cnt数组
...
int main(){
    ...
    insert(v[0],1);
    for(int i = 1; i <=  n ; i ++){
        if(i - m - 1 >=0 )insert(v[i - m - 1],-1);
        res = max(res,query(v[i]));
        insert(v[i],1);
    }
    cout<<res<<endl;
    return 0;
...
>这里如果我们求多次迪杰斯特拉的话会超时
>所以我们可以添加一个虚拟点，让他到所有商店的距离为0
>以虚拟点为原点，求一次迪杰斯特拉，然后dist[i] 就是i到
>虚拟点的距离，也就是到最近商店的距离

![无标题.png](https://cdn.acwing.com/media/article/image/2023/03/03/263183_3a09f50cb9-%E6%97%A0%E6%A0%87%E9%A2%98.png)



![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230330193328.png)
...
#include <queue>
const int N = 300010;
int g[N],e[N],w[N],ne[N],idx,dist[N],vist[N];
int n,m;
...
void dijkstra(){
   ...
   while(heap.size()){
       ...
       int distant  = k.first;
       int p = k.second;
       if(vist[p] == 1)continue;
       vist[p] = 1;
       for(int i = g[p]; i != -1 ; i = ne[i]){
           int j = e[i];
           int we = w[i];
           if(we + distant < dist[j]){
               dist[j] = we + distant;
               heap.push({dist[j],j});
           }
       }
...
>这道题的难点就是把二维坐标变成一维的，我们可以用哈希表来完成，然后再求最短路

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230504182705.png)
...
public:
    //添加边
    void add(int a,int b,map<pair<int,int>,int>&m){
        int n = m.size();
        if(m.count({a,b}) == 0)m[{a,b}] = n;
    }
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& specialRoads) {
        int mx = 100000000;
        //将二维坐标映射成一个点
        map<pair<int,int>,int>m;
        add(start[0],start[1],m);
        add(target[0],target[1],m);
        ...
        vector<int>line(n);
        vector<int>row(n);
        //记录每个点的横坐标和纵坐标,记录距离的值
        for(auto k : m){
            int a = k.second;
            int b = k.first.first;
            int c = k.first.second;
            line[a] = b;
            row[a] = c;
        }
        ...
        for(int i = 0 ;i < n ; i++){
            for(int j  = 0 ; j< n ; j ++){
                ...
                int c = line[j];
                int d = row[j];
                g[i][j] = min(abs(a - c) + abs(b - d),g[i][j]);
               // cout<<g[i][j]<<endl;
            }
        }
        for(auto k : specialRoads){
...
>题目意思
>给一组边，有向和无向边，求把无向边变成有向边构造无环图
>
>先判断有向边，用拓扑排序，如果可以拓扑，那么就可以构造，否则不能

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230401153729.png)
...
bool isTop(){
    ...
    while(l <= r){
        int k = q[l ++];
        for(int j = g[k] ; j != -1 ; j = ne[j]){
            degree[e[j]] --;
            if(degree[e[j]] == 0)q[++ r] = e[j];
        }
    }
    return r == n -1;
}
...
>因为要让全部a相等，所以让b2 == b3 == bn就行了
>所以求出有在最优解的情况下有多少b1
>我们分为选出l  和  r
>四种情况：
>
>+ l >= 2  并且r < =n   一次可以改变两个   一个加一个减  
>+ l == 1 r <= n  改变b1 和  br  ，一次改变一个
>+ l >= 2 并且 r == n  也是只改变一个 
>+ 另一种是l == 1 r == n + 1  没有影响
>
>所以我们求出b2 到bn之间的正数p和负数q,求和
>min(p,q)就是第一种情况的个数，然后差值就是其他的数量，两个相加就是操作次数
>
>方案数就是绝对值的差的绝对值

![微信截图_20230402170342](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230402170342.png)



![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230402193456.png)
...
![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230424130750.png)
...
vector<int> maxSlidingWindow(vector<int>& v, int m) {
    int n = v.size();
    vector<int>nums;
...
>思路，用单调队列维护最大值最小值就行了，每次更新窗口的大小

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230702174149.png)
    ...
    long long continuousSubarrays(vector<int>& nums) {
      ...
      for (int r = 0; r < nums.size(); ++r) {
        ...
        while (!mn.empty() && nums[r] < nums[mn.back()])
            ...
            mx.push_back(r);
            mn.push_back(r);
        while (!mx.empty() && !mn.empty() && nums[mx.front()] - nums[mn.front()] > 2) {
...
>首先我们可以知道后序遍历最后一个一定是根
>所以为什么可以递归构造
>构造的条件是中序遍历左右端点，后序遍历的左右端点
>然后递归构建左子树，递归构建右子树
>
>+ 左子树递归中，中序遍历il,和k - 1
>+ 后序遍历是pl 到x，可以解方程得到
...
![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230313202509.png)
...
>先预处理出每行，长度为b的最大值和最小值，保存到rmin和rmax中
>然后之后再根据每列，求这列长度为a的序列的最小值和最大值
>两次单调队列，然后求和就行了



![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230608212442.png)
...
> 思路
> 首先这是一棵树，分支之间没有干扰也没回路，所以可以用dp   dp(i)表示当前节点通电可以获取最大电容，dp(i) 等于其所有相邻点的dp之和
> 注意限制条件

![](https://gitee.com/hongshenghyj/picture/raw/master/img/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230630170224.png)
